...
f = open('Measures_Cascades_Rspread%s_MinimumCascadeSize%i.txt'%(Rspread,cascade_size),'w')
...
f = open('Cascades_Rspread%.6f_MinimumCascadeSize%i.txt'%(Rspread,cascade_size),'r')
f.readline()
line_num = -1
for line in f : 
    #if (line_num == 30) :
    ...
    # Save results
    f = open('Measures_Cascades_Rspread%s_MinimumCascadeSize%i.txt'%(Rspread,cascade_size),'a')
    f.write('\n%i\t%i\t%.4f'%(results[0],results[1],results[2]))
    f.close()
