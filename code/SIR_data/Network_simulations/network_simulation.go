...
func main() {
	...
	var network_name = "athletes_edges"

	// Important node: List of edges cannot list same edge twice (e.g. both 0,1 and 1,0 should not be listed.)
	...
	// Now define parameters for SIR model
	const R0 float64 = 0.55

	var mean_degree float64 = mean_degree(network)
	var rate_SI float64 = R0/(mean_degree -1)
	var rate_R float64 = 1

	// Define parameters for simulations
	const Nexp int = 10000
	var cascades_counted int = 0
	const Minimum_cascade_size int = 1
	//var filename string = "Cascades_on_network"+fmt.Sprintf("%f", network_name)+"_Rspread"+fmt.Sprintf("%f", R0)+"_MinimumCascadeSize"+strconv.Itoa(Minimum_cascade_size)+".txt"
	var filename string = "Cascades_on_network"+fmt.Sprintf("%s", network_name)+"_Rspread"+fmt.Sprintf("%f", R0)+"_MinimumCascadeSize"+strconv.Itoa(Minimum_cascade_size)+".txt"
	...
	for exp:= 0 ; exp<Nexp ; exp ++ {
		...
		//var seed int = draw_random_integer(len(network))
		...
		num_infected, num_SIedges, SI_arr, cascade_string = new_infection(num_infected,num_SIedges,SI_arr,cascade_string,network)

		// Now do the following until there are no more infectious nodes.
		for (num_infected>0) {
			if (draw_random_float()<float32(rate_SI*float64(num_SIedges)/(rate_SI*float64(num_SIedges)+float64(num_infected)*rate_R))) {
				num_infected, num_SIedges, SI_arr, cascade_string = new_infection(num_infected,num_SIedges,SI_arr,cascade_string,network)
				spread_total ++
			} else {
				num_infected, num_SIedges, SI_arr, cascade_string = new_removal(num_infected,num_SIedges,SI_arr,cascade_string,network)
			}
